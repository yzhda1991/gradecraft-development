module PerformerToolkit

  module SharedExamples
    RSpec.shared_examples "a cacheable resource" do |resource_name, expected_klass|
      let(:expected_klass) { resource_klass || resource_name.to_s.camelize.constantize }

      it "caches the #{resource_name}" do
        expect(performer).to receive(:"#{resource_name}").and_return send(resource_name.to_sym)
        subject
      end

      it "assigns the #{resource_name} to @#{resource_name}" do
        subject
        expect(performer.instance_variable_get(:"@#{resource_name}")).to eq(send(resource_name.to_sym))
      end

      it "caches an object that actually has the #{expected_klass} class" do
        subject
        expected_klass ||= resource_name.to_s.camelize.constantize
        expect(performer.instance_variable_get(:"@#{resource_name}").class).to eq(expected_klass)
      end
    end

    RSpec.shared_examples "a collection of cacheable resources" do |resource_name, resource_klass|
      let(:expected_klass) { resource_klass || resource_name.to_s.camelize.constantize }

      it "caches the #{resource_name}" do
        expect(performer).to receive(:"#{resource_name}").and_return send(resource_name.to_sym)
        subject
      end

      it "assigns the #{resource_name} to @#{resource_name}" do
        subject
        expect(performer.instance_variable_get(:"@#{resource_name}")).to eq(send(resource_name.to_sym))
      end

      it "caches objects that actually have the #{expected_klass} class" do
        subject
        performer.instance_variable_get(:"@#{resource_name}").each do |resource|
          expect(resourse.class).to eq(expected_klass)
        end
      end
    end
  end

end
