RSpec.describe InfoController, type: :controller, background_job: true do
  ...
  let(:professor) { create(:user) }
  let(:enroll_professor) { create(:course_membership, :professor, course_membership_attributes) }
  ...
  describe "#gradebook_file" do
    subject { get :gradebook_file, params: { id: course.id }}
