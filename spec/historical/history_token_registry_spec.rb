...
describe Historical::HistoryTokenRegistry do
  before { described_class.clear }

  describe ".register" do
    it "adds the token type to the registry" do
      described_class.register Historical::ActorHistoryToken, ->(key) { key == "actor_id" }

      expect(described_class.registered_tokens.length).to eq 1
      expect(described_class.registered_tokens.first.type).to eq Historical::ActorHistoryToken
    end
  ...
  describe ".unregister" do
    it "removes the token type from the registry" do
      described_class.register Historical::ActorHistoryToken, ->(key) { key == "actor_id" }
      described_class.unregister Historical::ActorHistoryToken
  ...
  describe ".for" do
    it "returns the tokens for the provided selector" do
      described_class.register Historical::ActorHistoryToken, ->(key, value, set) { key == "actor_id" }

      expect(described_class.for("actor_id", nil, nil).map(&:type)).to \
        eq [Historical::ActorHistoryToken]
    end
