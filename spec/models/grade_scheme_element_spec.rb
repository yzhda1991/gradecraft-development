...
describe GradeSchemeElement do
  ...
  context "validations" do
    ...
    it "is invalid without a low range" do
      subject.low_points = nil
      expect(subject).to be_invalid
    end

    it "is invalid without a high range" do
      subject.high_points = nil
      expect(subject).to be_invalid
    end
  ...
  describe "#range" do
    it "returns the difference between the high range and the low range" do
      subject.high_points = 100
      subject.low_points = 0
  ...
  describe "#within_range?" do
    subject { build(:grade_scheme_element, low_points: 1000, high_points: 1999) }
