...
describe HistoryTokenRegistry do
  ...
  describe ".register" do
    it "adds the token type to the registry" do
      described_class.register ActorHistoryToken, ->(key, value) { key == "actor_id" }

      expect(described_class.registered_tokens.length).to eq 1
      expect(described_class.registered_tokens.first.type).to eq ActorHistoryToken
    end
    ...
    it "adds the selector from the type if it responds to `tokenizable`" do
      class TestToken
        def self.tokenizable?(key, value); end
      end
  ...
  describe ".unregister" do
    it "removes the token type from the registry" do
      described_class.register ActorHistoryToken, ->(key, value) { key == "actor_id" }
      described_class.unregister ActorHistoryToken
  ...
  describe ".for" do
    it "returns the tokens for the provided selector" do
      ...
      expect(described_class.for("actor_id", nil).map(&:type)).to eq [ActorHistoryToken]
    end

    it "returns an empty array if the selector is not found" do
      described_class.register ActorHistoryToken, ->(key, value) { key == "actor_id" }

      expect(described_class.for("blah", nil)).to be_empty
    end
  end
end
