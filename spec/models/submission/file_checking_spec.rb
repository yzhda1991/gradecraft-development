...
describe "Submission file checking" do
  let(:submission) { build(:submission, submission_files: submission_files) }
  ...
  describe "#process_unconfirmed_files" do
    ...
    it "doesn't bother with confirmed files" do
      expect(unconfirmed_submission_file).not_to receive(:check_and_set_confirmed_status)
    end
  end
  ...
  describe "#confirm_all_files" do
    ...
    it "checks and sets confirmed status for confirmed files" do
      expect(unconfirmed_submission_file).to receive(:check_and_set_confirmed_status)
    end

    it "checks and sets confirmed status for unconfirmed files too" do
      expect(unconfirmed_submission_file).to receive(:check_and_set_confirmed_status)
    end
  end
end
