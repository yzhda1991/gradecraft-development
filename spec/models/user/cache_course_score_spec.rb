...
describe User do
  ...
  describe "#improved_cache_course_score", focus: true do
    ...
    context "course membership is present" do
      ...
      it "recalculates and updates the student score" do
        allow(course_membership).to receive(:recalculated_student_score) { 79000 }
        expect(course_membership).to receive(:recalculated_student_score)
        subject
      end

      it "updates the course membership with the recalculated score" do
        allow(course_membership).to receive(:recalculated_student_score) { 64000 }
        expect(course_membership).to receive(:update_attribute).with(:score, 64000)
        subject
      end

      it "returns the the recalculated student score" do
        allow(course_membership).to receive(:recalculated_student_score) { 58000 }
        expect(subject).to eq(58000)
      end
    end
