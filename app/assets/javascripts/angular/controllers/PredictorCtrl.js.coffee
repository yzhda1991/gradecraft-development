@gradecraft.controller 'PredictorCtrl', ['$scope', '$http', '$q', '$filter', 'PredictorService', ($scope, $http, $q, $filter, PredictorService) ->
  ...
  $scope.predictedGradeLevel = ()->
    ...
    _.each($scope.gradeSchemeElements,(gse)->
      if allPointsPredicted > gse.low_points
        if ! predictedGrade || predictedGrade.low_points < gse.low_points
          predictedGrade = gse
    )
    if predictedGrade
        ...
        if $scope.hasLevels(article)
          ...
          if $scope.inSnapRange(article,closest,ui.value)
            ...
            if articleType == 'assignment'
              angular.element("#assignment-" + article.id + "-level .value").text($filter('number')(closest.value) + " / " + $filter('number')(article.full_points))
            else
              angular.element("#challenge-" + article.id + "-level .value").text($filter('number')(closest.value) + " / " + $filter('number')(article.full_points))
          else
...
# GRAPHICS RENDERING
  ...
  $scope.renderGradeLevelGraphics = ()->
    ...
    g = svg.selectAll('g').data(gradeSchemeElements).enter().append('g')
            .attr("transform", (gse)->
              "translate(" + (scale(gse.low_points) + padding) + "," + 25 + " )")
            .on("mouseover", (gse)->
              d3.select(".grade_scheme-label-" + gse.low_points).style("visibility", "visible")
              d3.select(".grade_scheme-pointer-" + gse.low_points)
                .attr("transform","scale(4) translate(-.5,-3)")
                .attr("fill", "#68A127")
            )
            .on("mouseout", (gse)->
              d3.select(".grade_scheme-label-" + gse.low_points).style("visibility", "hidden")
              d3.select(".grade_scheme-pointer-" + gse.low_points)
    ...
    g.append("path")
      ...
      .attr("class",(gse)-> "grade_scheme-pointer-" + gse.low_points)
      .attr("transform","scale(2)")
    txt = d3.select("#svg-grade-level-text").selectAll('g').data(gradeSchemeElements).enter()
            .append('g')
            .attr("class", (gse)->
              "grade_scheme-label-" + gse.low_points)
    ...
    txt.insert("rect",":first-child")
      .attr("width", (gse)->
          angular.element(".grade_scheme-label-" + gse.low_points)[0].getBBox().width + (padding * 2)
        )
      .attr("height", 22)
      .attr("fill","#68A127")
    txt.attr("transform", (gse)->
      "translate(" + $scope.gradeLevelPosition(scale,gse.low_points,stats.width,padding) + "," + 0 + ")")
