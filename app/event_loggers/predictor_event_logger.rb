class PredictorEventLogger < ApplicationEventLogger
  ...
  # this defines filtering methods for all of the keys that will be taken out of the
  # params hash. If the value is a symbol, then a method will be defined using the same
  # name as the params key. If a hash is given, it will define a method using the name of
  # the hash value, and expects the same key name that was used in the params hash
